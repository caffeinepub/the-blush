import { useServices, useStaffMembers, useUpdateAppointmentStatus } from '../hooks/useQueries';
  ...
  if (appointments.length === 0) {
    return (
      <div className="text-center py-16 border border-border bg-card">
        <p className="font-serif text-xl text-muted-foreground">No appointments found.</p>
      </div>
    );
  }
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      {appt.status !== Status.confirmed && (
                        <button
                          onClick={() =>
                            updateMutation.mutate({ id: appt.id, status: Status.confirmed })
                          }
                          disabled={isUpdating}
                      ...
                      {appt.status !== Status.cancelled && (
                        <button
                          onClick={() =>
                            updateMutation.mutate({ id: appt.id, status: Status.cancelled })
                          }
                          disabled={isUpdating}
