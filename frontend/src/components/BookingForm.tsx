...
import { useServices, useStaffMembers, useBookAppointment } from '../hooks/useQueries';
...
const errorClass = 'font-sans text-xs text-destructive mt-1.5 flex items-center gap-1';
    ...
    return (
      <div className="border border-[var(--gold)]/30 bg-card p-10 text-center">
        <div className="w-14 h-14 mx-auto mb-6 flex items-center justify-center border border-[var(--gold)]/40">
          <CheckCircle size={24} strokeWidth={1.5} className="text-[var(--gold)]" />
        </div>
        ...
        <div className="text-left border border-border p-6 mb-8 space-y-3">
          <div className="flex justify-between">
            <span className="font-sans text-xs text-muted-foreground uppercase tracking-wide">Client</span>
            <span className="font-sans text-sm text-foreground font-medium">{form.clientName}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-sans text-xs text-muted-foreground uppercase tracking-wide">Service</span>
            <span className="font-sans text-sm text-foreground font-medium">{service?.name}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-sans text-xs text-muted-foreground uppercase tracking-wide">Stylist</span>
            <span className="font-sans text-sm text-foreground font-medium">{stylist?.name}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-sans text-xs text-muted-foreground uppercase tracking-wide">Date & Time</span>
          ...
          <div className="flex justify-between">
            <span className="font-sans text-xs text-muted-foreground uppercase tracking-wide">Booking ID</span>
            <span className="font-sans text-sm text-foreground font-medium">#{String(confirmedId)}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <button
            onClick={() => {
              setConfirmedId(null);
              setForm({ clientName: '', serviceId: '', staffId: '', date: '', time: '' });
            }}
